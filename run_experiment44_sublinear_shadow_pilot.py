"""Experiment 44 (README Follow-up 55; manuscript Discussion item 3,
"P0-1": a genuinely measurement-sublinear pilot). Compares the classical-
shadow pilot (`measurement_simulator_shadow.py`, validated in its own
self-test) against the direct-tomography pilot Follow-up 44/Ablation A
used throughout this project, at MATCHED total shot budgets, feeding
into the IDENTICAL downstream single-Pauli adaptive-recovery pipeline --
only the measurement-acquisition step differs.

The direct pilot splits its budget EVENLY across all 4^a-1=255 (n_a=4)
operators (`shots_per_setting = n_pilot // 255`); at very low budgets
this means some/most operators get very few shots (or, below n_pilot=
255, are not queried at all). The shadow pilot instead draws each shot
from ONE of 3^a=81 random bases and lets EVERY compatible operator
benefit from EVERY shot, regardless of budget. Budgets are swept from
well below 255 up to 1M to directly probe this low-budget regime, not
just the 10K-1M range used elsewhere in this project.

Usage:
    python run_experiment44_sublinear_shadow_pilot.py [--smoke]
"""
import argparse
import csv
import sys
...
N_REPEATS = 20
EXPERIMENT_ID = "paper13_exp44_shadow_pilot_v1"
...
def run(smoke: bool):
    pool = build_pool(N_A)
    out_path = RESULTS_DIR / "sublinear_shadow_pilot_comparison.csv"
    ...
    for seed in seeds:
        ...
        for n_pilot in budgets:
            direct_recoveries, shadow_recoveries = [], []
            ...
            print(f"seed={seed} n_pilot={n_pilot:>8} (shots/op direct={n_pilot//len(pool.all_matrices)}): "
                  f"direct={np.mean(direct_recoveries)*100:5.1f}% shadow={np.mean(shadow_recoveries)*100:5.1f}%",
                  flush=True)
...
def _summarize(rows, budgets):
    print("\n=== Direct tomography vs. classical-shadow pilot: mean recovery across seeds ===")
    for n_pilot in budgets:
        ...
        print(f"  n_pilot={n_pilot:>8}: direct={d*100:5.1f}%  shadow={sh*100:5.1f}%")
